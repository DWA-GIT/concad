...
public class Context {
	...
	public Context(IResource resource) {
		...
		variables = new HashMap<IBinding, List<VariableBinding>>();
		methods = new HashMap<MethodDeclaration, List<ASTNode>>();
		childrenContexts = new ArrayList<Context>();
	}
	...
	public VariableBinding addVariable(VariableBinding variableBinding) {
		...
		// 02 - If list does not contain this binding, it is the first time, so
		// we create it.
	...
	private void updateParentContextIfGlobalVariable(List<VariableBinding> vbs, VariableBinding variableBinding) {
		...
		// 02 - If this is a reference to a global variable, we also have to
		// update it.
	...
	private List<VariableBinding> getGlobalVariableBindings(VariableBinding variableBinding) {
		...
		List<VariableBinding> emptyList = new ArrayList<VariableBinding>();
	...
	public void addMethodDeclaration(MethodDeclaration method) {
		...
		if (!methods.containsKey(method)) {
			List<ASTNode> invocations = new ArrayList<ASTNode>();
	...
	private Map<IBinding, List<VariableBinding>> getGlobalVariables(Context otherContext, int type) {
		...
		Map<IBinding, List<VariableBinding>> tempVariables = new HashMap<IBinding, List<VariableBinding>>();

		// 01 - Iterate over all the variables.
		for (Entry<IBinding, List<VariableBinding>> entry : otherContext.getVariables().entrySet()) {

			// 02 - If the getKey is null, we probably have a syntax error. We
			...
			List<VariableBinding> currentVariableBindings = new ArrayList<VariableBinding>();
	...
	public List<VariableBinding> getVariableBindings(IBinding binding) {
		...
		if (null == vbs) {
			// 02 - If vbs == null, instead of returning null we return an empty
			// list.
